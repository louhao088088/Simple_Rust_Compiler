...
fn scalar_multiply_recursive(
    ...
    if (k % 2 == 0) {
        let half_point: EllipticPoint =
            scalar_multiply_recursive(k / 2, point, a, prime, depth + 1);
        return point_add(half_point, half_point, a, prime);
    } else {
        let reduced_point: EllipticPoint =
            scalar_multiply_recursive(k - 1, point, a, prime, depth + 1);
